...
assert.match(projectSource, /\{hasFinancialSummary \? \(/)
assert.doesNotMatch(projectSource, /isAnalyticsMode && hasFinancialSummary/)
assert.match(projectSource, /projectValue: paymentSummary\.projectValue/)
assert.match(projectSource, /amountPaid: paymentSummary\.totalPaid/)
assert.match(projectSource, /remainingBalance: paymentSummary\.outstandingBalance/)
assert.match(projectSource, /const projectSignedDate = resolvedContract\?\.signedDate/)
...
assert.match(projectSource, /navigate\(`\/invoices\/\$\{invoice\.id\}`(?:, \{ state: withNavigationContext\(\{\}, `\/projects\/\$\{currentLead\.id\}`)/)
