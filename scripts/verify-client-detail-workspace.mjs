...
assert.match(clientSource, /<RecordBackButton label=\{t\('back'\)\}/)
...
assert.match(clientSource, /project\.isProjectRecord[\s\S]*onOpenProject\(project\.id, \{ returnTo: `\/clients\/\$\{client\.id\}`/)
...
assert.equal(clientSource.match(/<RecordBackButton label=\{t\('back'\)\}/g)?.length, 1)
