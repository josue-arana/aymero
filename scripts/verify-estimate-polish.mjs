...
assert.match(estimatesSource, /hidden overflow-hidden rounded-2xl border border-slate-200 xl:block/)
assert.match(estimatesSource, /space-y-3 xl:hidden/)
assert.match(estimatesSource, /grid grid-cols-1 .*sm:grid-cols-\[minmax\(0,1fr\)_auto\]/)
assert.match(estimatesSource, /flex flex-nowrap gap-2 overflow-x-auto pb-1/)
assert.match(estimatesSource, /routeId = estimate\.id/)
assert.match(estimatesSource, /sumUnambiguousEstimateValues\(activeEstimates, projects\)/)
assert.match(estimatesSource, /optionName: estimate\.optionName/)
...
assert.doesNotMatch(estimateServiceSource, /Submitting contractor-scoped permanent estimate deletion/)
assert.doesNotMatch(estimateServiceSource, /Supabase permanent estimate deletion returned/)
assert.match(estimateServiceSource, /permanent estimate deletion affected no rows/)
assert.match(estimateServiceSource, /permanent estimate deletion failed/)
