...
function attachContractToLeadState(lead = {}, contract = null) {
  ...
  return {
    ...
    portal: {
      ...(lead?.portal || {}),
      estimate: linkedEstimate || lead?.portal?.estimate,
      contract: {
        ...(lead?.portal?.contract || {}),
        ...linkedContract,
...
function matchesLinkedEstimate(lead, estimate) {
  ...
  const leadId = resolveLinkedLeadId(lead, lead.id)
...
function matchesLinkedContract(lead, contract, estimate = null) {
  ...
  if (estimate?.id) {
    return Boolean((contract.estimateId || contract.estimate_id) && (contract.estimateId || contract.estimate_id) === estimate.id)
  }
  ...
  if (contract.estimateId || contract.estimate_id) {
    return (contract.estimateId || contract.estimate_id) === lead.estimateId
  }
...
function ContractorFlowApp() {
  ...
  const financialProjects = useMemo(() => persistedProjects.map((project) => {
      const projectId = String(project?.id || project?.projectId || project?.project_id || '')
      const projectInvoices = activeInvoices.filter((invoice) => String(invoice?.projectId || invoice?.project_id || '') === projectId)
      const projectInvoiceIds = new Set(projectInvoices.map((invoice) => String(invoice?.id || invoice?.invoiceId || invoice?.invoice_id || '')).filter(Boolean))
      return {
        ...project,
        invoices: projectInvoices,
        payments: persistedPayments.filter((payment) => (
          String(payment?.projectId || payment?.project_id || '') === projectId
            || projectInvoiceIds.has(String(payment?.invoiceId || payment?.invoice_id || ''))
        )),
      }
    }), [activeInvoices, persistedPayments, persistedProjects])
...
function buildWorkspaceJobRecord(job, clientRecord = null) {
  ...
  function createJob(job, clientRecord = null) {
    ...
    const linkedLeadId = nextJob.leadId || nextJob.lead_id || ''

    updateArchiveList('projectIds', nextJob.id, 'remove')
    updateArchiveList('deletedProjectIds', nextJob.id, 'remove')
    setPersistedProjects((current) => dedupeById([nextJob, ...current.filter((item) => item.id !== nextJob.id)]))

    if (linkedLeadId) {
      writeLeadPipelineStage(linkedLeadId, nextJob.leadPipelineStage)
      setLeads((current) => current.map((lead) => (
        lead.id === linkedLeadId
          ? {
              ...lead,
              projectId: nextJob.id,
              project_id: nextJob.id,
              projectStatus: nextJob.projectStatus,
              leadPipelineStage: nextJob.leadPipelineStage,
            }
          : lead
      )))
    }
  ...
  async function createJobFromModal(jobDraft) {
    ...
    try {
      ...
      if (response?.error) {
        showToast(t('jobSaveFailed'), 'error')
        logLeadDevError('[dev] Direct Project creation failed.', response.error, { jobDraft })
        return
      }
      ...
      showToast(t('jobSaveFailed'), 'error')
      logLeadDevError('[dev] Direct Project creation threw an unexpected error.', err, { jobDraft })
    }
  }
  ...
  function recordProjectPayment(projectId, payment) {
    const sourceProject = findProjectByLookup(persistedProjects, projectId, payment?.projectId, payment?.project_id)
    const sourceLead = findLeadByProjectLookup(leads, sourceProject?.leadId, sourceProject?.lead_id, payment?.leadId, payment?.lead_id)
    const portal = sourceLead?.portal || {}
    const relatedProjectId = payment?.projectId || payment?.project_id || sourceProject?.id || resolvePaymentProjectId(sourceLead)
    const paymentEntry = normalizePaymentRecord({
      id: payment?.id || `payment-${Date.now()}`,
      ...payment,
      clientId: payment?.clientId || payment?.client_id || sourceProject?.clientId || sourceProject?.client_id || sourceLead?.clientId || sourceLead?.client_id || null,
      projectId: relatedProjectId,
      leadId: payment?.leadId || payment?.lead_id || sourceProject?.leadId || sourceProject?.lead_id || sourceLead?.id || null,
    })
    setPersistedPayments((current) => dedupePayments([paymentEntry, ...current.filter((entry) => entry.id !== paymentEntry.id)]))
  ...
  function updateProjectPayment(projectId, payment) {
    const sourceProject = findProjectByLookup(persistedProjects, projectId, payment?.projectId, payment?.project_id)
    const sourceLead = findLeadByProjectLookup(leads, sourceProject?.leadId, sourceProject?.lead_id, payment?.leadId, payment?.lead_id)
    const portal = sourceLead?.portal || {}
    const relatedProjectId = payment?.projectId || payment?.project_id || sourceProject?.id || resolvePaymentProjectId(sourceLead)
  ...
  function deleteProjectPayment(projectId, payment) {
    const sourceProject = findProjectByLookup(persistedProjects, projectId, payment?.projectId, payment?.project_id)
    const sourceLead = findLeadByProjectLookup(leads, sourceProject?.leadId, sourceProject?.lead_id, payment?.leadId, payment?.lead_id)
    setPersistedPayments((current) => current.filter((entry) => entry.id !== payment?.id))

    if (!sourceLead) {
      showToast(t('paymentDeleted'))
      return
    }
  ...
  async function saveEstimate(leadId, estimate, { silent = false, createNew = false, leadOnly = false } = {}) {
    ...
    if (!sourceLead) {
      ...
      const directEstimateDraft = {
        ...(directEstimate || {}),
        ...estimate,
        id: nextEstimateId,
        contractorId: estimate?.contractorId || estimate?.contractor_id || sourceProject.contractorId || projectsContractorId || undefined,
        clientId: estimate?.clientId || estimate?.client_id || sourceProject.clientId || sourceProject.client_id || null,
        client_id: estimate?.clientId || estimate?.client_id || sourceProject.clientId || sourceProject.client_id || null,
      ...
      try {
        const response = directEstimate?.id
          ? await dataProvider.estimates.update(directEstimate.id, directEstimateDraft, { contractorId: projectsContractorId })
          : await dataProvider.estimates.create(directEstimateDraft, { contractorId: projectsContractorId })

        if (response?.error) {
          showToast(t('estimateSaveFailed'), 'error')
          logEstimateDevError('[dev] Direct-project estimate persistence failed.', response.error, {
            projectId: sourceProject.id,
            estimate: directEstimateDraft,
          })
          return null
        }
        ...
        showToast(t('estimateSaveFailed'), 'error')
        logEstimateDevError('[dev] Direct-project estimate save failed.', error, {
          projectId: sourceProject.id,
          estimate: directEstimateDraft,
        })
        return null
  ...
  async function createLeadEstimateOption(leadRecord, options = {}) {
    const lead = findLeadByProjectLookup(visibleLeads, leadRecord?.id, leadRecord?.leadId, leadRecord?.lead_id)
    if (!lead?.id) return null

    const currentEstimate = options.currentEstimate
    const currentEstimateName = String(options.currentEstimateName || '').trim() || null
    const previousCurrentEstimateName = String(currentEstimate?.optionName || currentEstimate?.option_name || '').trim() || null

    // The two-estimate handoff may rename the one existing estimate first. This
    // keeps estimate identity unchanged and only writes when the contractor
    // actually changed the optional human label.
    if (currentEstimate?.id && currentEstimateName !== previousCurrentEstimateName) {
      const renamed = await saveEstimate(lead.id, {
        ...currentEstimate,
        id: currentEstimate.id,
        optionName: currentEstimateName,
      }, { leadOnly: true, silent: true })
    ...
    draft.optionName = String(options.newEstimateName || '').trim() || null
    draft.option_name = draft.optionName
  ...
  async function findExistingLinkedContract({ sourceLead, estimateRecord = null, projectId = '', leadId = '', directProject = false } = {}) {
    ...
    const linkedContract = getLinkedActiveContract(sourceLead)
    const existingContract = hasContractData(persistedContract)
      ? persistedContract
      : estimateRecord?.id
        ? (linkedContract?.estimateId || linkedContract?.estimate_id) === estimateRecord.id ? linkedContract : null
        : linkedContract
    ...
    const estimateId = directProject ? null : estimateRecord?.id || sourceLead?.estimateId || sourceLead?.portal?.estimate?.id || null
    ...
    const leadMatches = leads.find((lead) => {
      ...
      if (estimateId) {
        return (contract.estimateId || contract.estimate_id) === estimateId || lead.estimateId === estimateId
      }
    ...
    for (const response of lookupResponses) {
      ...
      if (estimateId && nextContract && (nextContract.estimateId || nextContract.estimate_id) !== estimateId) {
        continue
      }
  ...
  function syncLeadContractState(leadId, contract, estimateRecord = null) {
    ...
    const contractProjectId = contract.projectId || contract.project_id || null
    if (contractProjectId) {
      const contractValue = toSafeNumber(contract.total ?? contract.totalAmount ?? contract.contractAmount)
      setPersistedProjects((current) => current.map((project) => (
        project.id === contractProjectId
          ? {
              ...project,
              value: contractValue || project.value,
              estimatedValue: contractValue || project.estimatedValue,
              contractValue: contractValue || project.contractValue,
              portal: {
                ...(project.portal || {}),
                contract: { ...(project.portal?.contract || {}), ...contract },
                contractAmount: contractValue || project.portal?.contractAmount || 0,
              },
            }
          : project
      )))
  ...
  async function ensureContractForLead(leadId, estimateInput = null, { directProject = false } = {}) {
    ...
    const contractPromise = (async () => {
      ...
      const projectEstimates = sourceProject ? getEstimatesForProject(sourceProject, persistedEstimates) : []
      const selectedProjectEstimate = sourceProject ? getSelectedEstimateForProject(sourceProject, projectEstimates) : null
      const baseEstimate = directProject
        ? null
        : hasEstimateData(estimateInput)
        ? estimateInput
        : sourceProject
      ...
      if (!directProject && !hasEstimateData(estimateInput) && projectEstimates.length > 1 && !selectedProjectEstimate) {
        showToast(t('estimateSelectionRequired'), 'error')
        return null
      }

      if (!directProject && sourceProject && hasEstimateData(estimateInput) && estimateInput?.id) {
        const exactProjectEstimate = projectEstimates.find((estimate) => estimate.id === estimateInput.id)
        const estimateProjectId = estimateInput.projectId || estimateInput.project_id || ''
        if (!exactProjectEstimate && (projectEstimates.length > 0 || (estimateProjectId && estimateProjectId !== sourceProject.id))) {
          showToast(t('estimateSelectionRequired'), 'error')
          return null
        }
      }

      if (hasEstimateData(baseEstimate)) {
        const estimateStatus = normalizeEstimateFinalizationStatus(baseEstimate.status)
        const estimateArchiveState = resolveEstimateArchiveState({ estimate: baseEstimate, lead: relationshipContext, archivedLeadIds: archives.leadIds })
        if (estimateArchiveState.isArchived || !canCreateContractFromEstimate(estimateStatus)) {
          showToast(t('contractSourceUnavailable'), 'error')
          return null
        }
      }
      ...
      const persistedEstimate = hasEstimateData(baseEstimate)
        ? await saveEstimate(leadId, {
            ...baseEstimate,
            projectId: resolvePersistedProjectLink(baseEstimate, relationshipContext),
            clientId: baseEstimate?.clientId || relationshipContext?.clientId || relationshipContext?.client_id || null,
          })
        : null

      const estimateRecord = persistedEstimate || baseEstimate || null
      const conversionResult = sourceLead && !directProject
        ? await ensureProjectForLeadConversion(sourceLead, {
            leadId,
            estimateRecord,
            silent: true,
          })
        : {
            linkedProjectId: sourceProject?.id || sourceLead?.projectId || sourceLead?.project_id || null,
            persistedLead: sourceProject || sourceLead,
            linkedEstimate: directProject ? null : estimateRecord,
            linkedClientRecord: clients.find((client) => client.id === (sourceProject?.clientId || sourceProject?.client_id || sourceLead?.clientId || sourceLead?.client_id)) || null,
          }
      ...
      const refreshedLead = conversionResult.persistedLead || findLeadByProjectLookup(leads, leadId) || relationshipContext
      ...
      const linkedClientId = conversionResult.linkedClientRecord?.id
        || refreshedLead?.clientId
        || refreshedLead?.client_id
        || linkedEstimateRecord?.clientId
        || linkedEstimateRecord?.client_id
        || null

      if (!linkedClientId) {
        showToast(t('contractRequiresClient'), 'error')
        return null
      }

      if (sourceProject && linkedEstimateRecord?.id && projectEstimates.length > 1) {
        const selectedEstimateId = sourceProject.selectedEstimateId || sourceProject.selected_estimate_id || ''
        if (selectedEstimateId !== linkedEstimateRecord.id) {
          const selectionResponse = await dataProvider.projects.update(sourceProject.id, {
            selectedEstimateId: linkedEstimateRecord.id,
            selected_estimate_id: linkedEstimateRecord.id,
          }, { contractorId: projectsContractorId })
          if (selectionResponse?.error) {
            showToast(selectionResponse.error.message || t('estimateSelectionRequired'), 'error')
            return null
          }
          setPersistedProjects((current) => current.map((project) => (
            project.id === sourceProject.id
              ? { ...project, ...(selectionResponse.data || {}), selectedEstimateId: linkedEstimateRecord.id, selected_estimate_id: linkedEstimateRecord.id }
              : project
          )))
        }
      }
      ...
      const contractPayload = {
        id: nextContractId,
        leadId: sourceLead?.id || sourceProject?.leadId || sourceProject?.lead_id || null,
        contractorId: projectsContractorId,
        clientId: linkedClientId,
        projectId: relatedProjectId,
        project_id: relatedProjectId,
        estimateId: directProject ? null : linkedEstimateRecord?.id || refreshedLead?.estimateId || null,
      ...
      try {
        ...
        if (response?.error) {
          showToast(t('contractSaveFailed'), 'error')
          logLeadDevError('[dev] Contract persistence failed.', response.error, {
            projectId: relatedProjectId,
        ...
        showToast(t('contractSaveFailed'), 'error')
        logLeadDevError('[dev] Contract persistence threw an unexpected error.', error, {
          projectId: relatedProjectId,
          leadId: sourceLead?.id || null,
        })
        return null
  ...
  function saveContract(leadId, contract) {
    ...
    setLeads((current) => current.map((lead) => {
      if (lead.id !== leadId) return lead
      const portal = lead.portal || {}
      persistedEstimate = hasEstimateData(portal.estimate) ? portal.estimate : null
  ...
  async function syncContractFromEstimate(leadId, estimateInput = null, { force = false } = {}) {
    const sourceLead = findLeadByProjectLookup(leads, leadId)
    const sourceProject = findProjectByLookup(persistedProjects, leadId, estimateInput?.projectId, estimateInput?.project_id)
    const relationshipContext = sourceLead || sourceProject
    ...
    const estimateRecord = persistedEstimate || baseEstimate || null
    const conversionResult = sourceLead
      ? await ensureProjectForLeadConversion(sourceLead, {
          leadId,
          estimateRecord,
          silent: true,
      ...
      : {
          ...
          persistedLead: sourceProject,
          linkedEstimate: estimateRecord,
          linkedClientRecord: clients.find((client) => client.id === (sourceProject.clientId || sourceProject.client_id)) || null,
        }
    ...
    const syncedContractPayload = {
      ...existingContract,
      leadId: sourceLead?.id || sourceProject?.leadId || sourceProject?.lead_id || existingContract.leadId || null,
  ...
  async function createInvoiceRecord(invoiceDraft) {
    try {
      const billingProject = persistedProjects.find((project) => String(project?.id || project?.projectId || project?.project_id || '') === String(invoiceDraft?.projectId || ''))
        || visibleLeads.find((lead) => String(lead?.projectId || lead?.project_id || lead?.id || '') === String(invoiceDraft?.projectId || ''))
        || {
        id: invoiceDraft?.projectId || '',
        clientId: invoiceDraft?.clientId || null,
        contractId: invoiceDraft?.contractId || null,
      }
      ...
      const billingError = validateInvoiceAgainstBillingCapacity({ capacity: billingCapacity, invoiceAmount: invoiceDraft?.amount })
      if (billingError) {
        throw new Error(t('invoiceExceedsAvailableToBill', { amount: currency.format(billingError.overage) }))
      }
  ...
  function openScheduleModal({ leadId = '', projectId = '', context = 'event', event = null, origin = '' } = {}) {
    setScheduleModalState({
      isOpen: true,
      leadId: leadId || event?.leadId || event?.lead_id || '',
      projectId: projectId || event?.projectId || event?.project_id || '',
      context,
      editingEvent: event,
      origin,
    })
  ...
  function closeScheduleModal() {
    setScheduleModalState({ isOpen: false, leadId: '', projectId: '', context: 'event', editingEvent: null, origin: '' })
  }
  ...
  function openProject(recordId, options = {}) {
    ...
    const matchingLead = findLeadByProjectLookup(visibleLeads, recordId)
    const routeId = matchingLead?.projectId || matchingLead?.project_id || ''

    if (!routeId) {
      navigate(`/leads/${matchingLead?.id || recordId}`)
      setSidebarOpen(false)
      return
    }
  ...
  function openEstimateForLead(leadId, estimateRecord = null, options = {}) {
    ...
    const matchingProject = findProjectByLookup(persistedProjects, estimateRecord?.projectId, estimateRecord?.project_id, leadId)
    const matchingLead = findLeadByProjectLookup(visibleLeads, leadId, matchingProject?.leadId, matchingProject?.lead_id)
    const hasProject = Boolean(matchingProject?.id || matchingLead?.projectId || matchingLead?.project_id)
  ...
  function openContractForLead(leadId, options = {}) {
    const matchingProject = findProjectByLookup(persistedProjects, options.projectId, leadId)
    const matchingLead = findLeadByProjectLookup(visibleLeads, leadId, matchingProject?.leadId, matchingProject?.lead_id)
  ...
  const routeElements = (
    <Routes>
      ...
      <Route path={appRoutes.projects} element={<ProjectRoute companySettings={companySettings} leads={visibleLeads} projects={persistedProjects} clients={clients} estimates={persistedEstimates} invoices={activeInvoices} scheduleEvents={visibleScheduleEvents} archivedIds={archives.projectIds} archivedScheduleEventIds={archives.scheduleEventIds} onBack={() => navigate('/dashboard')} onOpenPortal={openPortal} onOpenContract={openContractForLead} onConvertEstimate={async (leadId, estimate, options = {}) => { const contract = await ensureContractForLead(leadId, estimate, options); if (contract) openContractForLead(leadId, { source: 'project', projectId: contract.projectId || contract.project_id || undefined, leadId }) }} onCreateInvoice={(projectId) => openInvoiceModal({ projectId, lockProject: true, returnTo: `/projects/${projectId}`, returnLabelKey: 'backToProjectWorkspace' })} onMarkProjectComplete={markProjectComplete} onUpdateLead={updateLead} onRecordPayment={recordProjectPayment} onUpdatePayment={updateProjectPayment} onDeletePayment={deleteProjectPayment} onUploadPhotos={uploadProjectPhotos} onScheduleEvent={openScheduleModal} onExportEvent={exportScheduleEvent} onArchiveScheduleEvent={archiveRecord.scheduleEvent} onRestoreScheduleEvent={restoreRecord.scheduleEvent} onDeleteScheduleEvent={deleteRecord.scheduleEvent} onArchiveProject={archiveRecord.project} onRestoreProject={restoreRecord.project} onDeleteProject={deleteRecord.project} onCreateEstimateOption={createEstimateOption} onDuplicateEstimateOption={duplicateEstimateOption} onSelectEstimate={selectProjectEstimate} onClearEstimateSelection={clearProjectEstimateSelection} language={language} t={t} />} />
    ...
    <AnalyticsModeProvider settings={companySettings}>
      <div className="min-h-screen bg-[#f5f7fb] text-slate-950">
        ...
        <div className="lg:pl-[280px]">
          <div>
            <Topbar
              ...
              onOpenSubscription={() => navigate(appRoutes.subscription)}
        ...
        <ScheduleEventModal
          ...
          leads={activeLeads}
          projects={persistedProjects.filter((project) => !project.archivedAt && !project.archived_at)}
          clients={clients}
          initialLeadId={scheduleModalState.leadId}
          initialProjectId={scheduleModalState.projectId}
          ...
          onSave={async (event) => {
            ...
            try {
              ...
              showToast(t('eventUpdateFailed'), 'error')
...
function ProjectRoute({ companySettings, leads, projects = [], clients, estimates = [], invoices = [], scheduleEvents = [], archivedIds = [], archivedScheduleEventIds = [], onBack, onOpenPortal, onOpenContract, onConvertEstimate, onCreateInvoice, onMarkProjectComplete, onUpdateLead, onRecordPayment, onUpdatePayment, onDeletePayment, onUploadPhotos, onScheduleEvent, onExportEvent, onArchiveScheduleEvent, onRestoreScheduleEvent, onDeleteScheduleEvent, onArchiveProject, onRestoreProject, onDeleteProject, onCreateEstimateOption, onDuplicateEstimateOption, onSelectEstimate, onClearEstimateSelection, language, t }) {
  ...
  return (
      ...
      onScheduleEvent={() => onScheduleEvent?.({ projectId, leadId: leadRecordId, context: 'job' })}
      onEditScheduleEvent={(event) => onScheduleEvent?.({ projectId, leadId: leadRecordId, context: 'job', event })}
